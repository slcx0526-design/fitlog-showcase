...
function TrainInner() {
  ...
  return <div>
    <header className="mb-5">
      ...
      {!workout && !isPast && scheduled && <p className="inline-panel mt-3 text-[11px] text-muted">{t(`今日建议：${typeName[scheduled](mode)}。计划不会自动写入训练日志。`, `Suggested today: ${typeName[scheduled](mode)}. This will not be written to your workout log automatically.`, `今日の提案：${typeName[scheduled](mode)}。トレーニングログには自動で保存されません。`)}</p>}
    </header>
