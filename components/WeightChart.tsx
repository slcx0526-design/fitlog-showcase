...
import { rollingMean } from "@/lib/weight";
...
function sortedEntries(entries: BodyWeightEntry[]) {
...
export default function WeightChart({
  ...
  const weightRange = hi - lo || 1;

  const n = data.length;
  const x = (index: number) =>
    n === 1 ? W / 2 : PAD_X + (index * (W - PAD_X * 2)) / (n - 1);
  const y = (weight: number) =>
    PAD_TOP + (1 - (weight - lo) / weightRange) * (H - PAD_TOP - PAD_BOT);

  const linePath = data
    .map((entry, index) =>
      `${index === 0 ? "M" : "L"}${x(index).toFixed(1)},${y(entry.weight).toFixed(1)}`
    )
    .join(" ");
  ...
  const areaPath =
    ...
    data.map((entry, index) => `L${x(index).toFixed(1)},${y(entry.weight).toFixed(1)}`).join(" ") +
    ` L${x(n - 1).toFixed(1)},${(H - PAD_BOT).toFixed(1)} Z`;

  const latest = data[n - 1];
  const first = data[0];
  const rangeDelta = latest.weight - first.weight;
  const selectedX = selected ? x(selectedIndex) : null;
  const selectedY = selected ? y(selected.weight) : null;
    ...
    <div className="control-card p-3">
      <div className="mb-2 flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
        <div className="flex items-baseline gap-1.5">
          <span className="tnum text-[24px] font-bold text-fg">{latest.weight}</span>
        ...
        {n > 1 && (
          <div className="flex items-center gap-2">
            ...
            >
              {rangeDelta > 0 ? "+" : ""}
              {rangeDelta.toFixed(1)} kg · {tr("{n} 条", { n })}
            </span>
          </div>
        )}
      </div>
      ...
      <svg
        ...
        style={{ height: H }}
        role="img"
        aria-label={tr("体重趋势图：{range}，共 {n} 条记录", { range: rangeLabel, n })}
        onPointerDown={(event) => {
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-faint">
              {previous && selectedDelta != null
                ? `${tr("较上一条")} ${selectedDelta > 0 ? "+" : ""}${selectedDelta.toFixed(1)} kg`
                : tr("首条记录")}
            </p>
          </div>
          <span className="tnum ml-auto shrink-0 text-[18px] font-bold text-fg">
            {selected.weight}
            <span className="ml-0.5 text-[11px] font-normal text-faint">kg</span>
          </span>
        </div>
      )}
    </div>
