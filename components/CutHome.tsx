...
export default function CutHome() {
  ...
  function updatePlan(patch: Parameters<typeof setCutPlan>[0]) {
    if (setCutPlan(patch)) return;
    toast.show(localeText(locale, "减脂计划未能保存，请重试", "The cut plan could not be saved. Try again.", "減量計画を保存できませんでした。もう一度お試しください。"), { tone: "error" });
  }

  return <div className="space-y-4">
    <header className="page-heading"><div><p className="page-heading__eyebrow">{localeText(locale, "减脂计划", "Cut plan", "減量計画")}</p><h1>{tr("减脂")}</h1><p className="page-heading__meta">{tr("今天做什么，系统只给一条主线。")}</p></div><button type="button" onClick={() => updatePlan({ enabled: !active })} aria-label={tr(active ? "关闭减脂模式" : "开启减脂模式")} aria-pressed={active} className={"press relative mt-1 h-10 w-14 shrink-0 rounded-full transition-colors " + (active ? "bg-accent" : "border border-border bg-surface-2")}><span className={"absolute left-0 top-2 h-6 w-6 rounded-full bg-white shadow-sm transition-transform " + (active ? "translate-x-6" : "translate-x-1")} /></button></header>

    {!active ? <section className="control-card p-4"><p className="text-[16px] font-semibold text-fg">{tr("减脂模式未开启")}</p><p className="mt-1 text-[12px] leading-relaxed text-muted">{tr("开启后，饮食、有氧和体重趋势会汇总成每周速度。")}</p><button type="button" onClick={() => updatePlan({ enabled: true })} className="press mt-4 h-11 w-full rounded-xl bg-accent text-[13px] font-semibold text-accent-fg">{tr("开启减脂")}</button></section>
