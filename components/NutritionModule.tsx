...
export default function NutritionModule({ date, returnHref }: { date: string; returnHref?: string }) {
  ...
  return <section className="mt-1">
    ...
    <div className="control-card p-3.5">
      ...
      {empty && prev && <button onClick={copyPrevious} className="choice-chip press mt-3 flex h-10 w-full items-center justify-center border border-border bg-surface-2 text-[12px] font-semibold text-accent">复制上次真实记录 · {prev.calories} kcal</button>}
    ...
    {log.calories > 0 && returnHref && <button onClick={finish} className="press mt-3 h-12 w-full rounded-2xl bg-fg text-[15px] font-semibold text-bg">完成记录</button>}
  </section>;
}
