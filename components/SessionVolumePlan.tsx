...
import { cutSetPlan, DEFAULT_CUT_VOLUME_SCALE, isCutModeActive } from "@/lib/cutMode";
...
import type { Exercise, ExercisePreset, Template, TemplateItem, WorkoutSession } from "@/lib/types";

type SessionRow = { id: string; name: string; targetSets: number; currentSets: number; present: boolean; muscle?: string };
type MissingAction = { id: string; name: string; sets: number; suggestions: ExercisePreset[] };
...
export default function SessionVolumePlan({ date, workout }: { date: string; workout: WorkoutSession | undefined }) {
  ...
  const plan = useMemo(() => buildSessionPlan(template, exercises, pool, cutActive, scale), [template, exercises, pool, cutActive, scale]);
  ...
  return <section className={"session-volume-plan control-card overflow-hidden " + (gap ? "border-warn/35" : "")} data-session-volume-plan>
    <details open={gap > 0} data-session-volume-plan-details>
      ...
      <div className="soft-divider border-t px-3.5 pb-3 pt-3"><div className="grid grid-cols-3 gap-2"><Fact label={t("目标")} value={`${plan.targetSets} ${t("组")}`} accent /><Fact label={t("当前安排")} value={`${plan.currentPlannedSets} ${t("组")}`} warn={gap > 0} /><Fact label={t("已完成")} value={`${Math.min(plan.targetSets, plan.completedSets)} ${t("组")}`} /></div>{gap > 0 ? <div className="mt-3 rounded-xl bg-warn-soft px-3 py-2.5"><p className="text-[12px] font-semibold text-warn">{t("还有 {n} 组容量未覆盖", { n: gap })}</p><p className="mt-1 text-[10px] leading-relaxed text-muted">{t("缺失动作会按动作模式、目标肌群和器械替代库给出候选。加入后仍以本次容量为准，不会假装已经等价替代。")}</p>{plan.missing.map((missing) => <div key={missing.id} className="soft-divider mt-2 border-t border-warn/20 pt-2"><p className="text-[11px] font-semibold text-fg">{t(missing.name)} · {t("待覆盖 {n} 组", { n: missing.sets })}</p>{missing.suggestions.length ? <div className="mt-1.5 flex flex-wrap gap-1.5">{missing.suggestions.map((candidate) => <button key={candidate.id} type="button" onClick={() => addReplacement(candidate)} className="choice-chip press rounded-lg border border-border bg-surface px-2 py-1.5 text-[11px] font-semibold text-fg">{t("加入 {name}", { name: t(candidate.name) })}</button>)}</div> : <p className="mt-1 text-[10px] text-faint">{t("没有直接替代项；请从下方添加相近肌群动作。")}</p>}</div>)}</div> : <p className="mt-3 text-[10px] leading-relaxed text-faint">{t("当前安排已覆盖当天目标工作组。重量、次数和吨位不作为容量目标，由当天表现决定。")}</p>}<details className="mt-3 rounded-xl bg-surface-2 px-3 py-2.5"><summary className="cursor-pointer list-none text-[11px] font-semibold text-fg">{t("查看动作组数")}</summary><div className="soft-divider mt-2 space-y-1.5 border-t pt-2">{plan.rows.map((row) => <div key={row.id} className="flex items-center gap-2 text-[11px]"><span className="min-w-0 flex-1 truncate text-muted">{t(row.name)}{row.muscle ? ` · ${t(row.muscle)}` : ""}</span><span className={"tnum shrink-0 font-semibold " + (row.present ? "text-fg" : "text-warn")}>{row.present ? `${row.currentSets} / ${row.targetSets}` : t("待覆盖 {n} 组", { n: row.targetSets })}</span></div>)}</div></details></div>
    </details>
  </section>;
}

function buildSessionPlan(template: Template | undefined, exercises: Exercise[], pool: ExercisePreset[], cutActive: boolean, scale: number) { if (!template) return { targetSets: 0, currentPlannedSets: 0, completedSets: 0, missing: [] as MissingAction[], rows: [] as SessionRow[] }; const presets = new Map(pool.map((preset) => [preset.id, preset])); const sourceFor = (item: TemplateItem): ExercisePreset => presets.get(item.exerciseId) ?? { id: item.exerciseId, name: item.name, type: template.type, isMain: item.isMain ?? false, primaryMuscle: item.primaryMuscle, secondaryMuscles: item.secondaryMuscles, volumeContributions: item.volumeContributions, equipment: item.equipment, movementPattern: item.movementPattern, alternatives: item.alternatives, recordModes: item.recordModes }; const adjusted = new Map((cutActive ? cutSetPlan(template.items.map((item) => ({ id: item.exerciseId, sets: item.sets, isMain: item.isMain ?? presets.get(item.exerciseId)?.isMain })), scale) : template.items.map((item) => ({ id: item.exerciseId, cutSets: item.sets }))).map((row) => [row.id, row.cutSets])); const byId = new Map(exercises.map((exercise) => [exercise.id, exercise])); const existingIds = new Set(exercises.map((exercise) => exercise.id)); const rows = template.items.map((item: TemplateItem) => { const exercise = byId.get(item.exerciseId); const preset = sourceFor(item); return { id: item.exerciseId, name: item.name, targetSets: adjusted.get(item.exerciseId) ?? item.sets, currentSets: exercise ? plannedWorkingSets(exercise) || workingSets(exercise.sets).length : 0, present: Boolean(exercise), muscle: preset.primaryMuscle ? MUSCLE_LABELS[preset.primaryMuscle] : undefined }; }); const templateIds = new Set(template.items.map((item) => item.exerciseId)); const addedPlanned = exercises.filter((exercise) => !templateIds.has(exercise.id)).reduce((sum, exercise) => sum + (plannedWorkingSets(exercise) || workingSets(exercise.sets).length), 0); const currentTemplateSets = rows.reduce((sum, row) => sum + (row.present ? row.currentSets : 0), 0); const targetSets = rows.reduce((sum, row) => sum + row.targetSets, 0); const missing = rows.filter((row) => !row.present).map((row) => { const item = template.items.find((candidate) => candidate.exerciseId === row.id)!; return { id: row.id, name: row.name, sets: row.targetSets, suggestions: findAlternatives(sourceFor(item), pool, existingIds) }; }); return { targetSets, currentPlannedSets: currentTemplateSets + addedPlanned, completedSets: exercises.reduce((sum, exercise) => sum + workingSets(exercise.sets).length, 0), missing, rows }; }
function findAlternatives(source: ExercisePreset | undefined, pool: ExercisePreset[], existingIds: Set<string>) { if (!source) return []; const directIds = source.alternatives ?? []; const direct = directIds.map((id) => pool.find((item) => item.id === id)).filter((item): item is ExercisePreset => Boolean(item)); const similar = pool.filter((item) => item.id !== source.id && !directIds.includes(item.id) && (item.movementPattern === source.movementPattern || (source.primaryMuscle && item.primaryMuscle === source.primaryMuscle))); return [...direct, ...similar].filter((item) => !existingIds.has(item.id)).slice(0, 5); }
function Fact({ label, value, accent, warn }: { label: string; value: string; accent?: boolean; warn?: boolean }) { return <div className="rounded-xl bg-surface-2 px-2 py-2 text-center"><p className="text-[9px] text-faint">{label}</p><p className={"tnum mt-0.5 text-[12px] font-semibold " + (warn ? "text-warn" : accent ? "text-accent" : "text-fg")}>{value}</p></div>; }
