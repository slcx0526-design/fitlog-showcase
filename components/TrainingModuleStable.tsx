...
export default function TrainingModuleStable({ date, suggestedType }: { date: string; suggestedType?: TrainingType | null }) {
  ...
  function saveDifficulty(value: "easy" | "onTarget" | "hard") {
    if (workout?.difficulty === value) return;
    if (setWorkoutDifficulty(date, value)) return;
    toast.show(tx(locale, "本次整体感受未能保存，请重试", "Session effort could not be saved. Try again.", "今回の感覚を保存できませんでした。もう一度お試しください。"), { tone: "error" });
  }
