...
export default function BodyProgressReview() {
  ...
  function save() {
    if (!canSave) {
      toast.show(
        hasTodayRecord
          ? t("数值没有变化；修改后再更新。", "No values changed. Edit a value to update.", "数値は変わっていません。変更してから更新してください。")
          : t("请输入至少一项有效测量。", "Enter at least one valid measurement.", "有効な測定値を少なくとも1つ入力してください。"),
        { tone: "warning" },
      );
      return;
    }
  ...
  const latestWeights = useMemo(
    () => [...data.bodyWeights].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 5),
    [data.bodyWeights],
  );
  const latestWaists = useMemo(
    () => [...data.waistEntries].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 5),
    [data.waistEntries],
  );
  const actionLabel = canSave
    ? hasTodayRecord
      ? t("更新今天的测量", "Update today's measurements", "今日の測定値を更新")
      : t("保存测量", "Save measurements", "測定値を保存")
    : hasTodayRecord
      ? t("已保存 · 修改数值后更新", "Saved · edit a value to update", "保存済み · 数値を変更して更新")
  ...
  return <div className="space-y-4">
    <section className="progress-measure-panel control-card p-3.5">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-faint">{t("同一天可以反复更新，最新数值会覆盖旧记录。", "You can update repeatedly on the same day; the latest value replaces the earlier one.", "同じ日に何度でも更新でき、最新の値が以前の記録を上書きします。")}</p>
        </div>
        <span className="tnum shrink-0 rounded-full bg-surface-2 px-2 py-1 text-[11px] text-muted">
          {todayWeight != null ? `${t("体重", "Weight", "体重")} ✓` : t("体重", "Weight", "体重")} · {todayWaist != null ? `${t("腰围", "Waist", "ウエスト")} ✓` : t("腰围", "Waist", "ウエスト")}
        </span>
      </div>
    ...
    <section className="progress-trend-section">
      ...
      <div className="control-strip mb-2 grid grid-cols-3 gap-1 rounded-2xl p-1" aria-label={t("身体趋势指标", "Body trend metric", "身体推移の指標")}>
        {([
          { id: "weight", label: t("体重", "Weight", "体重") },
          { id: "waist", label: t("腰围", "Waist", "ウエスト") },
          { id: "bodyFat", label: t("体脂估算", "Body-fat estimate", "体脂肪推定") },
        ] as const).map((item) => <button key={item.id} type="button" onClick={() => setTrendMetric(item.id)} className={"choice-chip press h-9 text-[12px] font-semibold " + (trendMetric === item.id ? "bg-fg text-bg" : "text-muted")} aria-pressed={trendMetric === item.id}>{item.label}</button>)}
      </div>
