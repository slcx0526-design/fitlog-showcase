...
assert.equal(requiresCycleReviewBeforeWorkout(data, "2026-07-19"), false, "A backfill before the cycle-completing session must remain available");
assert.equal(requiresCycleReviewBeforeWorkout(data, "2026-07-20"), true);
assert.equal(requiresCycleReviewBeforeWorkout(data, "2026-07-21"), true, "A backfill after cycle completion must wait for review");
