...
assert.equal(hypertrophyArchive.sessionCount, 1, "An in-progress session must not enter the archive");
assert.deepEqual(hypertrophyArchive.sessions.map((session) => session.date), ["2026-07-02"]);
...
assert.equal(window.workingSets, 8);
assert.equal(window.completionCredits, 7.5);
assert.equal(window.planCredits, 6.5);
assert.equal(window.plannedSets, 7);
assert.equal(window.completionPct, 93);
...
assert.equal(dayHasLogContent({ date: "2026-07-06", workout: { type: "pull", exercises: [] } }), true);
