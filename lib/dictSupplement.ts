...
export const DICT_JA_SUPPLEMENT: Record<string, string> = {
  ...
  "训练 CSV": "トレーニング CSV", "身体 CSV": "身体 CSV", "日志 CSV": "ログ CSV", "备份与设置": "バックアップと設定", "模板": "テンプレート", "动作": "種目", "复制文字计划": "テキストプランをコピー", "删除模板": "テンプレートを削除",
  "数据结构正常": "データ構造は正常です", "发现 {n} 项数据需要整理": "整理が必要なデータが {n} 件あります", "{n} 次训练": "トレーニング {n} 回", "{n} 个有效组": "有効セット {n}", "自动整理": "自動整理", "已整理 {n} 项数据": "{n} 件のデータを整理しました", "保留 {n} 条旧轨道历史作为参考，不参与新轨道自动建议。": "旧トラック履歴 {n} 件は参照用に保持され、新トラックの自動提案には使用されません。", "重复的自定义动作标识": "重複したカスタム種目 ID", "重复的模板标识": "重複したテンプレート ID", "失效的周期模板绑定": "無効なサイクルテンプレート連携", "待统一的训练处方快照": "統一が必要なトレーニング処方スナップショット", "待统一的模板处方": "統一が必要なテンプレート処方", "异常的训练组数值": "異常なセット値", "重复日期的身体记录": "同日の身体記録の重複",
...
export const DICT_EN_SUPPLEMENT: Record<string, string> = {
  ...
  "训练 CSV": "Training CSV", "身体 CSV": "Body CSV", "日志 CSV": "Log CSV", "备份与设置": "Backup & settings", "模板": "Templates", "动作": "Exercises", "复制文字计划": "Copy text plan", "删除模板": "Delete template",
  "数据结构正常": "Data structure is healthy", "发现 {n} 项数据需要整理": "{n} data items need cleanup", "{n} 次训练": "{n} workouts", "{n} 个有效组": "{n} valid sets", "自动整理": "Clean up", "已整理 {n} 项数据": "Cleaned up {n} data items", "保留 {n} 条旧轨道历史作为参考，不参与新轨道自动建议。": "{n} legacy track records are kept as references and excluded from new-track suggestions.", "重复的自定义动作标识": "Duplicate custom exercise IDs", "重复的模板标识": "Duplicate template IDs", "失效的周期模板绑定": "Invalid cycle-template bindings", "待统一的训练处方快照": "Workout prescriptions awaiting normalization", "待统一的模板处方": "Template prescriptions awaiting normalization", "异常的训练组数值": "Invalid set values", "重复日期的身体记录": "Duplicate body entries for one date",
